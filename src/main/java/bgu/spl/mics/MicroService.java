...
public abstract class MicroService implements Runnable { 
    ...
    private MessageBusImpl mb = MessageBusImpl.getMessegeBus();
    HashMap<Class<? extends Message>, Callback> callBackPair;
    protected Diary diary= Diary.getDiary();
    ...
    public MicroService(String name) {
        this.callBackPair = new HashMap<Class<? extends Message>, Callback>();
    	this.name= name;
        mb.register(this);
    }
    ...
    mb.subscribeEvent(type, this);
        if(!callBackPair.containsKey(type))
            callBackPair.put(type, callback);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        mb.subscribeBroadcast(type, this);// might cause some problems.
        if(!callBackPair.containsKey(type))
            callBackPair.put(type, callback);
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        Future<T> myFuture=null;
        try {
             myFuture = mb.sendEvent(e);
    ...
    protected final void terminate() throws InterruptedException {
        //TODO change globalMessage name to terminationMessage
    	Broadcast termination = new terminationMessage();

    	this.sendBroadcast(termination);
    }
    ...
    public final void run() {
    	this.initialize();
        subscribeBroadcast(terminationMessage.class, (terminationMessage c)->{
            mb.unregister(this);
            Diary.getDiary().setTerminate(System.currentTimeMillis(),getName());
    ...
    while (true){
        ...
        Callback c = callBackPair.get(m.getClass());
                c.call(m);
        }
        }
